      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-12">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <div className="relative flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-blue-600 to-cyan-500 shadow-sm shadow-blue-500/20 group-hover:shadow-cyan-500/30 transition-all duration-300">
                <Snowflake className="w-4 h-4 text-white animate-cold-pulse" />
                <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500 to-cyan-400 rounded-lg blur-xs opacity-0 group-hover:opacity-60 transition duration-300 -z-10" />
              </div>
              <div className="flex flex-col">
                <div className="flex items-center gap-1.5">
                  <span className="text-base sm:text-lg font-black tracking-tight bg-gradient-to-r from-slate-900 via-blue-900 to-slate-800 dark:from-white dark:via-blue-200 dark:to-cyan-400 bg-clip-text text-transparent">
                    ColdTrack
                  </span>
                  <span className="inline-flex items-center px-1.5 py-0.2 rounded-md text-[9px] font-bold uppercase tracking-wider bg-blue-500/10 text-blue-600 dark:bg-blue-500/20 dark:text-cyan-300 border border-blue-500/20">
                    IoT Pro
                  </span>
                </div>
                <span className="hidden sm:block text-[9px] font-medium text-slate-500 dark:text-slate-400 tracking-wider">
            ...
            <div className="hidden lg:flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 dark:bg-emerald-950/40 dark:border-emerald-500/30">
          ...
          {isAuthenticated && (
            <div className="hidden md:flex items-center gap-1 bg-slate-100/80 dark:bg-slate-900/60 p-1 rounded-xl border border-slate-200 dark:border-slate-800/80">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`relative flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold transition-all duration-200 ${
                      isActive
                  ...
                  >
                    <Icon className={`h-3.5 w-3.5 ${isActive ? "text-blue-600 dark:text-cyan-200" : ""}`} />
                    <span>{item.name}</span>
                    {item.badge && (
                      <span className={`text-[9px] font-bold px-1 py-0.2 rounded-sm uppercase ${
                        isActive
